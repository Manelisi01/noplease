...
public class Tracker {
    ...
    // Data structures
    private static ConcurrentHashMap<String, PeerInfo> peers = new ConcurrentHashMap<>(); // Stores file hash to peer
                                                                                          // info mappings
        ...
        while (true) {
            ...
            switch (command) {
                case "REGISTER":
                    ...
                    // Add the seeder to the peers map
                ...
                case "UPDATE":
                    ...
                    // Find the peer and update its last seen timestamp
                    for (Map.Entry<String, PeerInfo> entry : peers.entrySet()) {
                        if (entry.getKey().equals(updateFileHash)) {
                            PeerInfo peerInfo = entry.getValue();
                            if (peerInfo.ip.equals(updateIP) && peerInfo.port == updatePort) {
